...
db = sqlite3.connect("db/microblog.db")
cur = db.cursor()
...
def settings_page():
  if "username" in session:
    bio = cur.execute("SELECT bio FROM users WHERE username = ?;", (session["username"], )).fetchone()[0]
...
def user_page(user):
  ...
  data = cur.execute("SELECT * FROM users WHERE username = ?;", (user, )).fetchone()
  if data == None:
...
def post_viewer(post):
  exists = int(cur.execute("SELECT COUNT(*) FROM posts WHERE id = ?;", (post, )).fetchone()[0]) > 0
...
def followers(user):
  data = [r[0] for r in cur.execute("SELECT follower FROM follows WHERE followed = ?;", (user, )).fetchall()]
...
def following(user):
  data = [r[0] for r in cur.execute("SELECT followed FROM follows WHERE follower = ?;", (user, )).fetchall()]
  if request.args.get("json") == "true":
    return data
...
def postdata(post):
  did_heart = False
  post_data = cur.execute("SELECT * FROM posts WHERE id = ?;", (post, )).fetchone()
  hearts = cur.execute("SELECT COUNT(DISTINCT user) FROM hearts WHERE parentId = ?;", (post, )).fetchone()[0]
  comments = cur.execute("SELECT * FROM comments WHERE parentId = ? ORDER BY id;", (post, )).fetchall()
  if "username" in session and int(cur.execute("SELECT COUNT(*) FROM hearts WHERE user = ? AND parentId = ?;", (session["username"],post_data[0])).fetchone()[0]) > 0:
    did_heart = True
  data = {
    "author": post_data[2],
    "content": post_data[3],
...
def get_posts():
  ...
  all_hearts = []
  if "username" in session:
    all_hearts = [row[0] for row in cur.execute("SELECT parentId FROM hearts WHERE user = ?;", (session["username"], )).fetchall()]
  posts = []
  order_by = "(SELECT COUNT(DISTINCT user) FROM hearts h WHERE h.parentId = id)" if sort == "top" else "id"
  if user == "all":
    posts = cur.execute(f"SELECT * FROM posts ORDER BY {order_by} DESC LIMIT ?, ?;", (start, count)).fetchall()
  elif user == "following" and "username" in session:
    cur.execute("ATTACH DATABASE 'db/users.db' AS user_stuff;")
    posts = cur.execute(f"SELECT * FROM posts WHERE author IN (SELECT followed FROM user_stuff.follows f WHERE f.follower = ?) ORDER BY {order_by} DESC LIMIT ?, ?;", (session["username"], start, count))
  elif user[0] == "@":
    posts = cur.execute(f"SELECT * FROM posts WHERE author = ? ORDER BY {order_by} DESC LIMIT ?, ?;", (user[1:], start, count)).fetchall()
  else:
    return error("Post feed error: invalid request"), 400
  posts = [{"id": post[0], "date": post[1], "author": post[2], "content": post[3], "hearts": cur.execute("SELECT COUNT(DISTINCT user) FROM hearts WHERE parentId = ?;", (post[0], )).fetchone()[0], "hearted": post[0] in all_hearts, "comments": cur.execute("SELECT COUNT(*) FROM comments WHERE parentId = ?;", (post[0], )).fetchone()[0]} for post in posts]
...
def create_post():
  ...
  id = int(cur.execute("SELECT max(id) FROM posts;").fetchone()[0] or 0) + 1
  cur.execute("INSERT INTO posts VALUES (?, ?, ?, ?);", (id, get_date(), username, sanitize(content)))
  db.commit()
...
def followaction():
  ...
  already_following = int(cur.execute("SELECT COUNT(*) FROM follows WHERE follower = ? AND followed = ?;", (username, to_follow)).fetchone()[0]) > 0
  if not already_following:
    cur.execute("INSERT INTO follows VALUES (?, ?);", (username, to_follow))
  else:
    cur.execute("DELETE FROM follows WHERE follower = ? AND followed = ?;", (username, to_follow))
  db.commit()
...
def heartaction():
  ...
  username = session["username"]
  if cur.execute("SELECT COUNT(*) FROM posts WHERE id = ?;", [postId]).fetchone()[0] == 0:
    return error("Heart failure: post does not exist")
  if session["username"] == cur.execute("SELECT author FROM posts WHERE id = ?;", [postId]).fetchone()[0]:
    return error("Heart failure: you cannot heart your own post")
  has_hearted = cur.execute("SELECT COUNT(*) FROM hearts WHERE parentId = ? AND user = ?;", (postId, username)).fetchone()[0] > 0
  if has_hearted:
    cur.execute("DELETE FROM hearts WHERE parentId = ? and user = ?;", (postId, username))
  else:
    cur.execute("INSERT INTO hearts VALUES (?, ?);", (postId, username))
  db.commit()
...
def create_comment():
  ...
  if cur.execute("SELECT COUNT(id) FROM posts WHERE id = ?;", (parentId)).fetchone()[0] != 1:
    return error("Comment creation failure: parent post does not exist"), 400
  id = int(cur.execute("SELECT MAX(id) FROM comments;").fetchone()[0] or 0) + 1
  cur.execute("INSERT INTO comments VALUES (?, ?, ?, ?, ?);", (id, date, parentId, author, sanitize(content)))
  db.commit()
...
def signup():
  ...
  if cur.execute("SELECT * FROM users WHERE username = ?;", (username, )).fetchone() is not None:
    return error("Signup failure: username already exists"), 400
  salt = bcrypt.gensalt()
  hashed = bcrypt.hashpw(password.encode("utf-8"), salt)
  cur.execute("INSERT INTO users VALUES(?, ?, ?, \"No bio provided.\");", (username, hashed.decode("utf-8")))
  db.commit()
...
def login():
  ...
  if not bcrypt.checkpw(password.encode("utf-8"), user_data[1].encode("utf-8")):
